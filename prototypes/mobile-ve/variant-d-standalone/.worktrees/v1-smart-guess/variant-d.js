...
// ============================================
// VERSION 1: SMART GUESS BANNER FUNCTIONALITY
// ============================================

function initSmartGuessBanner() {
  const banner = document.getElementById('smartGuessBanner');
  const acceptBtn = document.getElementById('acceptSmartGuess');
  const chooseOtherBtn = document.getElementById('chooseOtherType');

  if (!banner || !acceptBtn || !chooseOtherBtn) return;
  ...
  // Handle "Yes, use template" button
  acceptBtn.addEventListener('click', () => {
    // Hide banner with animation
    banner.classList.add('hidden');

    // Wait for animation then auto-trigger template selection
    setTimeout(() => {
      // Find the Society topic
  ...
  // Handle "Choose different" button
  chooseOtherBtn.addEventListener('click', () => {
    // Hide banner
    banner.classList.add('hidden');

    // Open category selector (same as pressing "/")
    setTimeout(() => {
      openCategorySelector();
    }, 300);
  });
...
if (!window.__variantDInitialized) {
  ...
  initSmartGuessBanner(); // VERSION 1: Initialize smart guess banner
}
