...
// === VERSION 2: FLOATING SMART BAR LOGIC ===

function initFloatingSmartBar() {
  const smartBar = document.getElementById('floatingSmartBar');
  const selector = document.getElementById('articleTypeSelector');
  const canvas = document.getElementById('canvas');

  if (!smartBar || !selector || !canvas) return;

  // Simulate Wikidata lookup result for "Tiger Cricket Academy"
  const wikidataMatch = {
    topic: 'Society',
    ...
    icon: '🏏',
    label: 'Cricket organization'
  };

  let templateSelected = false;

  // Handle clicking the selector to change article type
  selector.addEventListener('click', () => {
    openCategorySelector();
  });

  // Auto-load template on page load after brief delay
  setTimeout(() => {
    const societyTopic = WIKIPEDIA_CATEGORIES.find(t => t.name === 'Society');
    if (societyTopic) {
      const organizationsSubcat = societyTopic.subcategories.find(s => s.name === 'Organizations');
      if (organizationsSubcat) {
        // Organizations doesn't have granular types, use selectCategory directly
        // Call selectCategory for Organizations
          selectCategory(societyTopic, organizationsSubcat);
  ...
  // Add gentle pulse animation if user starts typing without template
  let hasTyped = false;
  canvas.addEventListener('input', () => {
    if (!hasTyped && !templateSelected) {
      hasTyped = true;
      smartBar.style.animation = 'pulse 0.5s ease-in-out 3';
    }
  });
}

// === VERSION 3: CONVERSATIONAL DIALOGUE LOGIC ===

function initConversationalDialogue() {
  const dialogue = document.getElementById('conversationalDialogue');
  const yesBtn = document.getElementById('dialogueYes');
  const noBtn = document.getElementById('dialogueNo');
  const canvas = document.getElementById('canvas');

  if (!dialogue || !yesBtn || !noBtn || !canvas) return;
  ...
  // Handle "Yes, that's right" button
    ...
    setTimeout(() => {
      ...
      if (societyTopic) {
        ...
        if (organizationsSubcat) {
          ...
          // Call selectCategory for Organizations
            selectCategory(societyTopic, organizationsSubcat);
            // Remove dialogue after animation
            setTimeout(() => {
  ...
  // Focus canvas after dialogue appears
  setTimeout(() => {
    canvas.focus();
  }, 600);
}

// === VERSION 4: ZERO-TAP INTELLIGENCE LOGIC ===

function initZeroTapIntelligence() {
  const canvas = document.getElementById('canvas');
  const toast = document.getElementById('contextToast');
  const toastDismiss = document.getElementById('toastDismiss');

  if (!canvas || !toast || !toastDismiss) return;

  let templateLoaded = false;
  let toastShown = false;

  // Silently load template in background after brief delay
  setTimeout(() => {
    const societyTopic = WIKIPEDIA_CATEGORIES.find(t => t.name === 'Society');
    if (societyTopic) {
      const organizationsSubcat = societyTopic.subcategories.find(s => s.name === 'Organizations');
  ...
  // Show toast only on first user interaction
  function showToast() {
    if (!toastShown && templateLoaded) {
      toastShown = true;
  ...
  canvas.addEventListener('focus', showToast, { once: true });
  canvas.addEventListener('click', showToast, { once: true });
  canvas.addEventListener('input', showToast, { once: true });

  // Manual dismiss button
  toastDismiss.addEventListener('click', dismissToast);
}

// === VERSION 5: HYBRID SLASH + UPFRONT LOGIC ===

function initHybridShortcuts() {
  const shortcuts = document.getElementById('smartShortcuts');
  const organizationChip = document.getElementById('shortcutOrganization');
  const browseChip = document.getElementById('shortcutBrowse');
  const canvas = document.getElementById('canvas');

  if (!shortcuts || !organizationChip || !browseChip || !canvas) return;

  // Handle smart shortcut click (organization template)
  organizationChip.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      if (societyTopic) {
        const organizationsSubcat = societyTopic.subcategories.find(s => s.name === 'Organizations');
        if (organizationsSubcat) {
          // Organizations doesn't have granular types, use selectCategory
          selectCategory(societyTopic, organizationsSubcat);
          // Remove shortcuts after animation
          setTimeout(() => {
  ...
  // Auto-hide shortcuts if user starts typing (preserves "/" command)
  let hasTyped = false;
  canvas.addEventListener('input', () => {
    if (!hasTyped) {
      hasTyped = true;
      const text = canvas.textContent.trim();
      // If user types "/" don't hide shortcuts yet
      if (text !== '/') {
        setTimeout(() => {
